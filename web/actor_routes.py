...
async def actors_directory_page(req):
    ...
    <style>
        ...
        .s-row-2 {{ display: flex; gap: 10px; flex-wrap:nowrap; align-items:center; overflow-x:auto; }}
...
async def create_actor_page(req):
    ...
    content = '''
<style>
.ca-label{font-size:11px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:1px;margin-bottom:6px;margin-top:14px;display:block}
.ca-input{width:100%;background:var(--bg3);border:1px solid var(--border);padding:12px 16px;color:var(--text);border-radius:8px;outline:none;font-family:inherit;font-weight:600;font-size:14px;transition:border-color .2s;margin-bottom:2px}
.ca-input:focus{border-color:var(--accent)}
.ca-textarea{width:100%;background:var(--bg3);border:1px solid var(--border);padding:12px 16px;color:var(--text);border-radius:8px;outline:none;font-family:inherit;font-weight:600;font-size:14px;min-height:100px;resize:vertical;transition:border-color .2s}
.ca-textarea:focus{border-color:var(--accent)}
.ca-cdd-wrap{position:relative;margin-bottom:2px}
...
.ca-cdd-menu{position:absolute;top:calc(100% + 6px);left:0;right:0;background:var(--bg2);border:1.5px solid var(--border);border-radius:12px;overflow:hidden;z-index:9999;box-shadow:0 8px 32px rgba(0,0,0,.45);display:none}
...
.ca-link-url-wrap{display:none;margin-top:8px}
.ca-link-url-wrap.show{display:block}
</style>

<form action="/api/create_actor" method="post" enctype="multipart/form-data">
  <input type="hidden" name="category" id="ca_cat_val" value="actor">
  <input type="hidden" name="link_type" id="ca_link_type_val" value="none">

  <span class="ca-label">Full Name</span>
  <input type="text" name="name" placeholder="Full Name (Actor / App / Website)" class="ca-input" required>

  <span class="ca-label">Select Category</span>
  <div class="ca-cdd-wrap">
    <div class="ca-cdd-btn" id="ca_cat_btn" onclick="toggleCACdd('cat',event)">
      <span id="ca_cat_lbl">🎭 Actor Profile</span>
    </div>
    <span class="ca-cdd-arrow">&#9660;</span>
    <div class="ca-cdd-menu" id="ca_cat_menu">
      <div class="ca-cdd-item selected" onclick="pickCACat('actor','🎭 Actor Profile',this,event)">🎭 Actor Profile<span class="ca-radio"><span class="ca-radio-dot"></span></span></div>
      <div class="ca-cdd-item" onclick="pickCACat('app','📱 App Profile',this,event)">📱 App Profile<span class="ca-radio"><span class="ca-radio-dot"></span></span></div>
      <div class="ca-cdd-item" onclick="pickCACat('website','🌐 Website Profile',this,event)">🌐 Website Profile<span class="ca-radio"><span class="ca-radio-dot"></span></span></div>
    </div>
  </div>

  <span class="ca-label">Biography / Description</span>
  <textarea name="bio" placeholder="Biography / Description Details..." class="ca-textarea" required></textarea>

  <span class="ca-label">Search Tags (Comma Separated)</span>
  <input type="text" name="tags" placeholder="e.g. SRK, Netflix, Mod" class="ca-input">

  <span class="ca-label">Link Type</span>
  <div class="ca-cdd-wrap">
    <div class="ca-cdd-btn" id="ca_link_btn" onclick="toggleCACdd('link',event)">
      <span id="ca_link_lbl">🚫 No Link</span>
    </div>
    <span class="ca-cdd-arrow">&#9660;</span>
    <div class="ca-cdd-menu" id="ca_link_menu">
      <div class="ca-cdd-item selected" onclick="pickCALink('none','🚫 No Link','',this,event)">🚫 No Link<span class="ca-radio"><span class="ca-radio-dot"></span></span></div>
      <div class="ca-cdd-item" onclick="pickCALink('instagram','📸 Instagram','https://instagram.com/',this,event)">📸 Instagram<span class="ca-radio"><span class="ca-radio-dot"></span></span></div>
      <div class="ca-cdd-item" onclick="pickCALink('facebook','👥 Facebook','https://facebook.com/',this,event)">👥 Facebook<span class="ca-radio"><span class="ca-radio-dot"></span></span></div>
      <div class="ca-cdd-item" onclick="pickCALink('twitter','🐦 Twitter / X','https://x.com/',this,event)">🐦 Twitter / X<span class="ca-radio"><span class="ca-radio-dot"></span></span></div>
      <div class="ca-cdd-item" onclick="pickCALink('website','🌐 Website','https://',this,event)">🌐 Website<span class="ca-radio"><span class="ca-radio-dot"></span></span></div>
      <div class="ca-cdd-item" onclick="pickCALink('other','🔗 Other','https://',this,event)">🔗 Other<span class="ca-radio"><span class="ca-radio-dot"></span></span></div>
    </div>
  </div>
  <div class="ca-link-url-wrap" id="ca_link_url_wrap">
    <input type="text" name="link_url" id="ca_link_url_input" placeholder="Paste full URL here..." class="ca-input" style="margin-top:0">
  </div>

  <span class="ca-label">Profile Photo / Icon</span>
  <input type="file" name="photo" accept="image/*" required style="padding:10px 0; color:var(--text); width:100%;">

  <button class="submit-btn" type="submit" style="background:var(--accent);color:#fff;width:100%;padding:14px;border:0;border-radius:6px;font-weight:700;cursor:pointer;margin-top:16px;">Create Profile</button>
</form>

<div style="margin-top:15px;text-align:center;">
  <a href="/actors" style="color:var(--muted);text-decoration:none;font-size:13px;">← Back to Catalog</a>
</div>

<script>
function toggleCACdd(type,e){
  e.stopPropagation();
  var menu=document.getElementById('ca_'+type+'_menu');
  var btn=document.getElementById('ca_'+type+'_btn');
  var isOpen=menu.style.display==='block';
  document.querySelectorAll('.ca-cdd-menu').forEach(function(m){m.style.display='none';});
  document.querySelectorAll('.ca-cdd-btn').forEach(function(b){b.classList.remove('open');});
...
function pickCACat(val,lbl,el,e){
  e.stopPropagation();
  document.getElementById('ca_cat_val').value=val;
  document.getElementById('ca_cat_lbl').innerText=lbl;
  document.querySelectorAll('#ca_cat_menu .ca-cdd-item').forEach(function(i){i.classList.remove('selected');});
  el.classList.add('selected');
  document.getElementById('ca_cat_menu').style.display='none';
  document.getElementById('ca_cat_btn').classList.remove('open');
}
function pickCALink(val,lbl,placeholder,el,e){
  e.stopPropagation();
  document.getElementById('ca_link_type_val').value=val;
  document.getElementById('ca_link_lbl').innerText=lbl;
  document.querySelectorAll('#ca_link_menu .ca-cdd-item').forEach(function(i){i.classList.remove('selected');});
  el.classList.add('selected');
  document.getElementById('ca_link_menu').style.display='none';
  document.getElementById('ca_link_btn').classList.remove('open');
  var urlWrap=document.getElementById('ca_link_url_wrap');
  var urlInput=document.getElementById('ca_link_url_input');
  if(val==='none'){urlWrap.classList.remove('show');urlInput.value='';}
  else{urlWrap.classList.add('show');if(!urlInput.value||urlInput.value==='https://'||urlInput.value.startsWith('https://'))urlInput.value=placeholder;}
}
</script>
'''
...
async def api_create_actor(req):
    ...
    try:
        ...
        while True:
            ...
            elif part.name == 'link_type': link_type = (await part.read()).decode().strip()
            elif part.name == 'link_url': link_url = (await part.read()).decode().strip()
        ...
        await actors.insert_one({"name": name, "bio": bio, "category": category, "tags": [t.strip() for t in tags_raw.split(",") if t.strip()], "photo_url": f"TG_ID:{tg_photo_id}", "is_actor_permanent": True, "photo_updated_at": now_ts, "social_links": {"instagram": "", "youtube": "", "twitter": ""}, "link_type": link_type, "link_url": link_url if link_type != "none" else "", "gallery": [], "is_gallery_permanent": True, "created_at": now_ts})
...
async def api_actor_update_profile(req):
    ...
    await actors.update_one({"_id": ObjectId(actor_id)}, {"$set": {
        ...
        "link_type": d.get('link_type', 'none').strip(),
        "link_url": d.get('link_url', '').strip() if d.get('link_type', 'none') != 'none' else ""
    }})
